        ...
        if w*h > MAX_PIXELS:
            ...
            if len(matched) == len(flat):
                ...
                for col,cnt in top:
                    ...
                    st.color_picker("", value=hexc, disabled=True, label_visibility="collapsed")
            st.image(img.resize((w*display_zoom,h*display_zoom),Image.NEAREST),caption="Crisp Enlarged Image",use_container_width=False)
...
# --- Tool 2: Pixel Deleter / Adder ---
    ...
    if uploaded:
        ...
        if st.sidebar.button("Reset"): st.session_state.current = st.session_state.orig.copy(); st.session_state.undo=[]
        if st.sidebar.button("Undo") and st.session_state.undo: st.session_state.current = st.session_state.undo.pop()

        chain = st.sidebar.checkbox("Chain effects")
        base  = st.session_state.current if chain else st.session_state.orig.copy()
        h,w   = base.shape[:2]
        mode  = st.sidebar.selectbox("Pattern/Mode:",["Original","Checkerboard","Alt Rows","Alt Cols","Diagonal","H Stripes","V Stripes","Random Mask","Rings","Border","Grid","Pixel Adder"])
        ...
        if mode == "Pixel Adder":
            ...
            if not maintain:
                ...
                if dist == "Uniform":
                    ...
                    offsets = rng.integers(-dup, dup+1, size=(h, w))
                    cols = np.clip(np.arange(w)[None, :] + offsets, 0, w-1)
                    rows = np.arange(h)[:, None]
                    preview = base[rows, cols]

                elif dist == "Column":
                    w_e = exp.shape[1]
                    cols = rng.choice(w_e, size=w, replace=False)
                    cols.sort()
                    preview = exp[:, cols, :]
                    ...
                    for i in range(h):
                        ...
                        for _ in range(int(w * pct)):
                            ...
                            picks.append(j)
                            rm = range(max(0, j-dup), min(w, j+dup)+1)
                            available = [x for x in available if x not in rm]
                        for j in picks:
                ...
                rng = np.random.default_rng(seed)
                # pick w columns out of expanded width
                h_e, w_e = expanded.shape[:2]
        ...
        st.image(disp.resize((disp.width*display_zoom,disp.height*display_zoom),Image.NEAREST),caption="Preview")
