    ...
    if uploaded:
        ...
        if mode == "Pixel Adder":
            ...
            if not maintain:
                ...
                # initialize offsets for each pixel
                if dist == "Uniform":
                    pct  = st.sidebar.slider("% pixels offset", 0, 100, 50)
                    mask = rng.random((h, w)) < (pct/100.0)
                    offsets = np.zeros((h, w), dtype=int)
                    # random integer offsets in [-dup,dup]
                    offsets[mask] = rng.integers(-dup, dup+1, size=mask.sum())
                else:  # Column uniform shift
                    col_shift = rng.integers(-dup, dup+1)
                    offsets   = np.full((h, w), col_shift, dtype=int)
                # construct preview by shifting base horizontally
                rows, cols = np.indices((h, w))
                new_cols   = np.clip(cols + offsets, 0, w-1)
                preview    = base[rows, new_cols]
        else:
                seed = st.sidebar.number_input("Random seed", 0)
                rng = np.random.default_rng(seed)
                dist = st.sidebar.selectbox("Distribution:", ["Uniform", "Column"])
