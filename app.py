    ...
    if uploaded:
        ...
        if mode == "Pixel Adder":
            ...
            if mode == "Original":
                ...
                seed = int(st.sidebar.number_input("Seed", 0))
                rng = np.random.default_rng(seed)
                mask = (rng.random((h, w)) >= pct / 100.0)

            elif mode == "Rings":
                R = st.sidebar.slider("Ring thickness", 1, max(1, min(h, w) // 4), 10)
