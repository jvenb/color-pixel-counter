...
# --- Tool 2: Pixel Deleter / Adder ---
    ...
    if uploaded:
        ...
        # --- PIXEL ADDER WITH MAINTAIN WIDTH ---
        if mode == "Pixel Adder":
                    ...
                    for i in range(h):
                        for j in range(w):
                            if rng.random() < pct/100.0:
                                ...
                                pick = dup
                            # exp row i spans columns [0..w*(2*dup+1)-1]
                            block = exp[i, j*(2*dup+1):(j+1)*(2*dup+1)]
                            preview[i,j] = block[pick]
                else:  # Column mode
                    col_pick = rng.integers(0, 2*dup+1)
                    for i in range(h):
                        for j in range(w):
                            block = exp[i, j*(2*dup+1):(j+1)*(2*dup+1)]
                            preview[i,j] = block[col_pick]

        # --- ALL OTHER DELETER PATTERNS ---
        else:
...
# --- Tool 3: Rubik Mosaic Checker ---
    ...
    if inv_file and tgt_file:
        ...
        for c, cnt in Counter(tgt_map.flatten()).items():
            st.write(f"- {c}: {cnt}")
        buf = BytesIO()
        Image.fromarray(dispB).save(buf,format="PNG"); buf.seek(0)
