...
# --- Mode 2: Pixel Deleter ---
    ...
    if uploaded:
        ...
        if pattern=="Checkerboard": inv=st.sidebar.checkbox("Invert checkerboard",False); mask=np.fromfunction(lambda y,x:((x+y)%2==(1 if inv else 0)),(h,w))
        ...
        preview=base.copy(); preview[...,3]*=mask.astype(np.uint8)
        ...
        disp=Image.fromarray(preview).resize((w*display_zoom,h*display_zoom),Image.NEAREST); st.image(disp,caption="Image Preview",use_container_width=False)
...
# — Mode 3: Rubik Mosaic Checker —
    ...
    if invariant_file and target_file:
        ...
        if inv_img.size!=tgt_img.size: tgt_img=tgt_img.resize(inv_img.size,Image.NEAREST)
        h,w=inv_img.size[1],inv_img.size[0]
        inv_arr=np.array(inv_img); tgt_arr=np.array(tgt_img)
        inv_map=np.empty((h,w),dtype=object); tgt_map=np.empty((h,w),dtype=object)
        for y in range(h):
            for x in range(w):
                inv_map[y,x]=nearest_rubik_color(tuple(inv_arr[y,x])); tgt_map[y,x]=nearest_rubik_color(tuple(tgt_arr[y,x]))
        cy,cx=h//2,w//2
        if tgt_map[cy,cx]!=inv_map[cy,cx]: st.error(f"Invariant violated: center must be {inv_map[cy,cx]}")
        else: st.success("Center invariant holds.")
        violations=[]
        for name,opp in opposites.items():
            m1=(tgt_map==name); m2=(tgt_map==opp)
            for y in range(h):
                for x in range(w-1):
                    if m1[y,x] and m2[y,x+1]: violations.append(((x,y),(x+1,y),name,opp))
        if violations: st.warning(f"Found {len(violations)} opposite-color adjacency violation(s).")
        else: st.success("No opposite-color adjacency violations.")
        disp_arr=np.zeros((h,w,3),dtype=np.uint8)
        for y in range(h):
            for x in range(w): disp_arr[y,x]=rubik_colors[tgt_map[y,x]]
        ...
        for color,cnt in Counter(tgt_map.flatten()).items(): st.write(f"- {color}: {cnt}")
