...
# --- Tool 2: Pixel Deleter / Adder ---
    ...
    if uploaded:
        ...
        # --- Pixel Adder with maintain-width offset logic ---
        if mode == "Pixel Adder":
            ...
            if not maintain:
                ...
                dist = st.sidebar.selectbox("Distribution:",
                                            ["Uniform","Column"])
                    ...
                    for i in range(h):
                        for j in range(w):
                            if rng.random() < pct:
                                ...
                                offset = 0
                            new_col = np.clip(j + offset, 0, w-1)
                            preview[i,j] = base[i,new_col]
                else:  # Column
                    offset = rng.integers(-dup, dup+1)
                    for i in range(h):
                        for j in range(w):
        ...
        # --- All other patterns (deleter) ---
            ...
            if mode == "Checkerboard":
                ...
                M   = st.sidebar.slider("Stripe height M",
                                        1, h//2, 10)
                ...
                M   = st.sidebar.slider("Stripe width M",
                                        1, w//2, 10)
                ...
                R   = st.sidebar.slider("Ring thickness",
                                        1, min(h,w)//4, 10)
                ...
                K   = st.sidebar.slider("Border width K",
                                        0, min(h,w)//2, 10)
